...
# ==========================================
# Exact colors extracted from your provided image
COLOR_MAP = {
    'Crossword': '#5a4687', 'Bingo': '#216f96', 'Spin the Wheel': '#39a4a1',
    'Crossword Paradise': '#0f8558', 'Race 6': '#75ab48', 'Spin Roulette': '#edb210',
    'Terdrup': '#e4791c', 'Pick 3': '#cb5443', 'Lotto': '#9a3a64',
    'Pick 4': '#76436d', 'Free Roll': '#888888'
}

# Changed to LIGHT TEMPLATE for black text visibility
LIGHT_TEMPLATE = dict(
...
def main():
    ...
    html, body, [class*="css"] { font-family: 'Sora', sans-serif; color: #000000; }
    ...
    .metric-card .label { font-size: 12px; color: #475569; text-transform: uppercase; letter-spacing: 1px; font-weight: 700; margin-bottom: 4px; }
    .metric-card .value { font-size: 28px; font-weight: 700; color: #000000; }
    .metric-card .delta-pos { font-size: 13px; font-weight: 600; color: #059669; margin-top: 2px; }
    .metric-card .delta-neg { font-size: 13px; font-weight: 600; color: #dc2626; margin-top: 2px; }
    .metric-card .delta-neu { font-size: 13px; font-weight: 600; color: #475569; margin-top: 2px; }
    ...
    .scorecard-name  { font-size: 14px; font-weight: 700; color: #000000; margin-bottom: 6px; }
    .scorecard-total { font-size: 22px; font-weight: 700; color: #000000; }
    .scorecard-sub   { font-size: 12px; font-weight: 600; color: #475569; margin-top: 2px; }
    .scorecard-share { font-size: 12px; font-weight: 600; color: #475569; margin-top: 4px; }
    .rank-badge      { display:inline-block; background: #e2e8f0; border-radius:6px; padding: 2px 8px; font-size:11px; color:#000000; margin-right:6px; font-weight: bold;}
    ...
    <div style='padding: 10px 0 20px 0;'>
        <div style='font-size:32px; font-weight:800; color:#000000; letter-spacing:-0.5px;'>
    ...
    with tab1:
        ...
        for row in rows:
            ...
            for idx, product in enumerate(row):
                ...
                mom_color= "#059669" if mom_val >= 0 else "#dc2626"
        ...
        with col_rank:
            ...
            fig_rank = go.Figure(go.Bar(
                ...
                marker=dict(color=[COLOR_MAP.get(p) for p in totals['Product']]),
                text=totals['Sales'], texttemplate='$%{text:,.0f}', textposition='outside',
                textfont=dict(size=13, color='black', weight='bold') # Enforced Black Text
            ))
            fig_rank.update_layout(
                title=dict(text="Total Revenue Ranking", font=dict(color='black', size=16, weight='bold')), 
                **LIGHT_TEMPLATE, height=420,
                xaxis=dict(showgrid=True, gridcolor='#e2e8f0'),
                yaxis=dict(showgrid=False, tickfont=dict(color='black', weight='bold')),
        ...
        with col_pie:
            ...
            fig_pie = px.pie(
                ...
                hole=0.45, title="Revenue Market Share"
            )
            # Pushed text outside so black text is highly visible on white background
            ...
            fig_pie.update_layout(
                title=dict(font=dict(color='black', size=16, weight='bold')),
                **LIGHT_TEMPLATE, height=420, showlegend=False, 
        ...
        if selected_deep:
            ...
            fig_deep.add_trace(go.Bar(
                ...
                name='Daily Revenue', marker_color=p_color, opacity=0.8
            ))
            fig_deep.add_trace(go.Scatter(
                x=deep_df['Date'], y=deep_df['7DMA'],
                name='7-Day MA', line=dict(color='#000000', width=3) # Black trend line
            ))
            fig_deep.update_layout(
                title=dict(text=f"{selected_deep} — Daily Revenue + Moving Average", font=dict(color='black', weight='bold')),
                **LIGHT_TEMPLATE, height=380,
    ...
    with tab2:
        ...
        fig_area.update_layout(
            title=dict(text="Cumulative Daily Revenue (Stacked)", font=dict(color='black', weight='bold')),
            **LIGHT_TEMPLATE, height=400,
        ...
        with col_bar:
            ...
            fig_monthly.update_layout(
                ...
                **LIGHT_TEMPLATE, height=380, legend=dict(orientation='h', y=-0.2), yaxis=dict(gridcolor='#e2e8f0')
            )
    ...
    with tab3:
        ...
        with t_col1:
            ...
            fig_trend.add_trace(go.Scatter(
                ...
                name='Daily Total', mode='lines', line=dict(color='#94a3b8', width=2)
            ))
            fig_trend.add_trace(go.Scatter(
                x=df_filtered['Date'], y=df_filtered['7DMA_Total'],
                name='7-Day MA', line=dict(color='#000000', width=3)
            ))
            fig_trend.update_layout(
                title=dict(text="Total Revenue Trend", font=dict(color='black', weight='bold')),
                **LIGHT_TEMPLATE, height=350, legend=dict(orientation='h', y=1.1), yaxis=dict(gridcolor='#e2e8f0')
            )
        ...
        with t_col2:
            ...
            fig_dow.update_layout(
                title=dict(font=dict(color='black', weight='bold')),
                **LIGHT_TEMPLATE, height=350, yaxis=dict(gridcolor='#e2e8f0'), legend=dict(orientation='h', y=-0.25)
            )
        ...
        fig_weekly = px.line(
            ...
            title="Weekly Revenue Breakdown", color_discrete_map=COLOR_MAP
        )
        fig_weekly.update_traces(line=dict(width=3))
        fig_weekly.update_layout(
            title=dict(font=dict(color='black', weight='bold')),
            **LIGHT_TEMPLATE, height=380, xaxis=dict(tickangle=-45), yaxis=dict(gridcolor='#e2e8f0'), legend=dict(orientation='h', y=-0.3)
        )
    ...
    with tab4:
        ...
        with mi_col1:
            ...
            fig_winners.update_traces(textfont=dict(color='white', weight='bold')) # White text inside solid bars is ok
            fig_winners.update_layout(title=dict(font=dict(color='black', weight='bold')), **LIGHT_TEMPLATE, height=370, showlegend=False, yaxis=dict(gridcolor='#e2e8f0'))
        ...
        with mi_col2:
            ...
            fig_pareto.add_trace(go.Bar(x=cumulative['Product'], y=cumulative['Revenue'], name='Revenue', marker_color=[COLOR_MAP.get(p,'#ccc') for p in cumulative['Product']]), secondary_y=False)
            fig_pareto.add_trace(go.Scatter(x=cumulative['Product'], y=cumulative['Cumulative %'], name='Cumulative %', line=dict(color='#000000', width=3)), secondary_y=True)
            
            fig_pareto.update_layout(
                title=dict(text="Pareto Revenue Analysis", font=dict(color='black', weight='bold')),
                **LIGHT_TEMPLATE, height=370, legend=dict(orientation='h', y=1.12), yaxis=dict(gridcolor='#e2e8f0')
            )
        ...
        fig_scatter = px.scatter(
            ...
            title="Product Efficiency Matrix", color_discrete_map={r['Product']: r['Color'] for _, r in stats_df.iterrows()}
        )
        fig_scatter.update_traces(textposition='top center', textfont=dict(color='black', weight='bold'))
        fig_scatter.update_layout(
            title=dict(font=dict(color='black', weight='bold')),
            **LIGHT_TEMPLATE, height=450, showlegend=False, xaxis=dict(gridcolor='#e2e8f0'), yaxis=dict(gridcolor='#e2e8f0')
        )
    ...
    with tab5:
        ...
        st.markdown("Ask me anything about your sales data. Try: *'Compare Lotto and Bingo'*, *'Overall product trends'*, *'Show monthly'*, *'Trend analysis'*")

        user_query = st.text_input("Your question:", placeholder="e.g. Compare Lotto and Bingo")
        ...
        if user_query:
            ...
            if "compare" in q or " vs " in q:
                ...
                if len(found_products) >= 2:
                    ...
                    fig_c.update_layout(title=dict(font=dict(color='black', weight='bold')), **LIGHT_TEMPLATE, yaxis=dict(gridcolor='#e2e8f0'))
                ...
                fig_t.add_trace(go.Scatter(x=df_filtered['Date'], y=df_filtered['7DMA'], name='7-Day Avg', line=dict(color='#000000', width=3)))
                fig_t.update_layout(title=dict(font=dict(color='black', weight='bold')), **LIGHT_TEMPLATE, yaxis=dict(gridcolor='#e2e8f0'))
                st.plotly_chart(fig_t, use_container_width=True)

            elif any(p.lower() in q for p in active_cols):
                found = [p for p in active_cols if p.lower() in q][0]
                fig_sp = px.line(df_filtered, x='Date', y=found, title=f"{found} Daily Revenue", color_discrete_sequence=[COLOR_MAP[found]])
                fig_sp.update_layout(title=dict(font=dict(color='black', weight='bold')), **LIGHT_TEMPLATE, yaxis=dict(gridcolor='#e2e8f0'))
                st.plotly_chart(fig_sp, use_container_width=True)
            else:
                st.write("I'm not sure how to answer that yet. Try comparing products or asking for trends.")
